...
send "echo '=== ИСПРАВЛЕНИЕ АКТИВНОЙ КОНФИГУРАЦИИ ==='\r"
expect "=== ИСПРАВЛЕНИЕ АКТИВНОЙ КОНФИГУРАЦИИ ==="
send "sed -i 's/localhost:3011/127.0.0.1:8080/g' /etc/nginx/sites-available/birdwatching\r"
expect "root@hiplet-41895:/opt/birdwatching#"
send "echo '=== ПРОВЕРКА ИЗМЕНЕНИЙ ==='\r"
expect "=== ПРОВЕРКА ИЗМЕНЕНИЙ ==="
send "grep -A 5 -B 5 'proxy_pass.*8080' /etc/nginx/sites-available/birdwatching\r"
expect "root@hiplet-41895:/opt/birdwatching#"
